...
class _BackSetState extends State<StyleSet> {
  ...
  final ValueNotifier<bool> lockOrCustomColorNotifier =
  ...
  final ValueNotifier<int> pickColorValueNotifier =
      ValueNotifier<int>(Colors.white.value);
  ...
  void dispatchColorValue(int colorValue) {
    ...
    switch (colorTypeOptionNotifier.value) {
      ...
      case 4:
        {
          eventBus
              .fire(ChangeSearchBarBorderColorEvent(colorValue: colorValue));
  ...
  int getColorValue() {
    ...
    switch (colorTypeOptionNotifier.value) {
      case 0:
        {
          return indexDB.get(StoreKey.pageBackColorValue) as int? ??
              defaultColorValue;
        }
      case 1:
        {
          return indexDB.get(StoreKey.settingBtnColorValue) as int? ??
              defaultColorValue;
        }
      case 2:
        {
          return indexDB.get(StoreKey.searchBtnColorValue) as int? ??
              Colors.deepPurpleAccent.value;
        }
      case 3:
        {
          return indexDB.get(StoreKey.searchBarTextColorValue) as int? ??
              Colors.black.value;
        }
      case 4:
        {
          return indexDB.get(StoreKey.searchBarBorderColorValue) as int? ??
              Colors.deepPurpleAccent.value;
        }
      case 5:
        {
          return indexDB.get(StoreKey.searchBarBackColorValue) as int? ??
  ...
  Widget build(BuildContext context) {
    return CommonSet(
      title: '样式',
      clearCallback: () async {
        await indexDB.delete(StoreKey.searchBarAlignOption);
        await indexDB.delete(StoreKey.searchBarLengthOption);
        await indexDB.delete(StoreKey.boxFitOption);
        await indexDB.delete(StoreKey.lockOrCustomColor);
        await indexDB.delete(StoreKey.pageBackColorValue);
        await indexDB.delete(StoreKey.settingBtnColorValue);
        await indexDB.delete(StoreKey.searchBtnColorValue);
        await indexDB.delete(StoreKey.searchBarTextColorValue);
        await indexDB.delete(StoreKey.searchBarBorderColorValue);
        await indexDB.delete(StoreKey.searchBarBackColorValue);
                        ...
                        ValueListenableBuilder<int>(
                          ...
                          builder: (context, colorTypeOption, child) {
                            return CustomDropdown(
                              ...
                              onChanged: (String? newValue) {
                                final int temp =
                                    colorTypeOptions.indexOf(newValue!);
